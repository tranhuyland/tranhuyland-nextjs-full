...
import { X, ShieldCheck, MapPin, Calendar, Shield, Phone } from 'lucide-react';
...
export default function Modals({ selectedProduct, onClose, tinhThoiGianCachDay, layMangHinhAnh }) {
  ...
  // Khởi tạo các biến theo dõi cử chỉ vuốt chạm trên iPhone
  ...
  // KHÔI PHỤC TÍNH NĂNG VUỐT ĐỂ BACK (SWIPE TO CLOSE) CHO IPHONE
  useEffect(() => {
    ...
    const handleTouchEnd = (e) => {
      // Nếu đang vuốt thanh trượt ảnh thì không kích hoạt đóng modal
      ...
      // Nếu vuốt từ trái qua phải một khoảng > 70px và không bị lệch xéo dọc quá mức
      if (khoangCachX > 70 && khoangCachY < 45) {
        onClose();
      }
    };
    ...
    <>
      ...
      {selectedProduct && (
        <div className="fixed inset-0 z-40 bg-slate-900/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4">
          ...
          >
            {/* ĐÃ SỬA: Đưa nút dấu X chuẩn của thư viện Lucide vào góc phải trên cao để người dùng tắt nhanh */}
            <button 
              onClick={onClose} 
              ...
              <div className="relative aspect-[16/10] bg-slate-100 image-slider-container">
                <div className="w-full h-full flex overflow-x-auto snap-x snap-mandatory no-scrollbar">
                  {danhSachAnh.map((url, idx) => (
                    <img key={idx} src={url} className="w-full h-full object-cover flex-shrink-0 snap-start" alt="Hình thực tế bất động sản" />
                  ))}
                </div>
              </div>
              <div className="p-6">
